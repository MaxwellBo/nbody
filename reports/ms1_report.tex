...
\begin{enumerate}
    ...
    \item \texttt{--nodes} (\texttt{1, 4, 8, 12})
    \item \texttt{--ntasks} (\texttt{1, 4, 8, 12})
    \item \texttt{--cpus-per-task} (\texttt{1, 4, 16})
    \item \texttt{ENABLE\_BARNES\_HUT} (\texttt{true, false})
\end{enumerate}
...
All valid combinations\footnote{As an example, \texttt{TASKS = 1, NODES=12} is not an admissable configuration, as the number of tasks must be greater than the number of nodes} of the values in the lists above were performed. Tweaks in the number of bodies ($n$) (a step from 4 to 16 bodies is double the body density; tests with 16384 bodies did not terminate), \texttt{--nodes} (\texttt{dogmatix} only has 12 nodes in the \texttt{cosc} partition; it did not make sense to disable MPI) and \texttt{--ntasks} (it made sense allocate the same number of tasks and nodes) were made. \texttt{--ntasks-per-node} was removed as a tested variable, as it is dynamically set by Slurm. The OMP scheduling policy was unable to be set by program argument, and would have require three seperate binaries compiled with different options - thus, this was also removed as a tested variable.
