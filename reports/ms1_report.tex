...
All valid combinations\footnote{As an example, \texttt{TASKS = 1, NODES=12} is not an admissable configuration} of the values in the lists above were performed. Tweaks in the number of bodies (a step from 4 to 16 bodies is double the body density; tests with 16384 bodies did not terminate), the number of nodes (\texttt{dogmatix} only has 12 nodes in the \texttt{cosc} partition; it did not make sense to disable MPI), the number of tasks (it made sense allocate the same number of tasks and nodes)
